...
//                            qDebug() << "SendAttack:" << (LPCVOID)sigScan.launcher.fun.SendAttack.address;
...
//                            qDebug() << "AutomaticWeapon:" << (LPCVOID)sigScan.launcher.ins.AutomaticWeapon.address << "\n";
                        ...
                        else if (threads->typeGame == 0)
                        {
                            ...
                            if(threads->section[threads->typeGame].miscSetting.isUnhookCamera == true)
                            {
                                if(pachFun[threads->typeGame].isUnhookCamera == true)
                                {
                                    ...
                                    DWORD ptrCameraFly = ToolsHack::FindDMAAddy(_blockpost->pHandle, _blockpost->baseAddress.Controll, { 0x5C, 0xEC });
                                    if ((LPCVOID)ptrCameraFly != nullptr && (LPCVOID)ptrCameraFly != (LPCVOID)0xEC)
                                    {
                                        _blockpost->player.fly = 1;
                                        WriteProcessMemory(_blockpost->pHandle, (LPCVOID*)(ptrCameraFly), &_blockpost->player.fly, 4, nullptr);
                                        pachFun[threads->typeGame].isUnhookCamera = false;
                        ...
                        qDebug() << (LPCVOID)_blockpost->baseAddress.MatrixBegin;
                        ...
                        {
                            DWORD ptrGamemode= ToolsHack::FindDMAAddy(_blockpost->pHandle, _blockpost->baseAddress.Controll, { 0x5C, 0x208 });
                            ReadProcessMemory(_blockpost->pHandle, (LPCVOID)(ptrGamemode), &_blockpost->server.gamemode, 4, nullptr);
                        }
